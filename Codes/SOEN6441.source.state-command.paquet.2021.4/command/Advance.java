...
public class Advance implements Order {
	...
	public void execute() {
		if (valid()) {
			if (target.owner.equals(initiator)) {
				...
				if (this.initiator.state != Player.State.pacific) {
	...
	public boolean valid() {
		if (source.owner.equals(initiator) 
				&& this.num_to_advance <= source.numArmies 
				&& !(!target.owner.equals(initiator) && initiator.state == Player.State.pacific))
			return true;
		else {
